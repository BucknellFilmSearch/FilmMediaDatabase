...
export default class ContextDialog extends React.Component {
   ...
   * @param {*} checked The new state of the checkbox's toggle
   */
  toggleBoundingBoxes(event, checked) {
    // The key and object to be put in
    const boxKey = `${this.props.currentFilm.movieOclcId}-${this.props.currentScreenshot.movieLineNumber}`;
    const boxes = {};

    if(this.state.showBoundingBoxes) {
      // Swap into the disabled key
      boxes[`${boxKey}-disabled`] = this.state.boxes[`${boxKey}`];
    } else {
      // Swap into the enabled key
      boxes[`${boxKey}`] = this.state.boxes[`${boxKey}-disabled`];
    }
  // Update state
  this.setState({showBoundingBoxes: checked, boxes }, () => console.log(this.state));
  }
  ...
  retrieveBoundingBoxes(oclcId, lineNo) {
    ...
    .then((res) => {
      const boxes = {};
      // Depending on whether bounding boxes are enabled or disabled, display them
      if (this.state.showBoundingBoxes) {
